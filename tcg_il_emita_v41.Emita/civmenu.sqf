...
if ((_loopart == "arrest") and (player distance prisonflag <= 100))  then 
...
(format ['if (rolestring == "%1") then {[0, 0, 0, ["inventorycheck", %2, %3, %4]] spawn inventory_invcheck;};',_aktionsStarter, INV_LizenzOwner, INV_InventarArray, player]) call broadcast;
...
(format ['if (rolestring == "%1") then {[0, 0, 0, ["licensecheck", %2, %3, %4]] spawn cop_licensecheck;};',_aktionsStarter, INV_LizenzOwner, INV_InventarArray, player]) call broadcast;
