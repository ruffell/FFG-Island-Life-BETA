...
_CopOnlyShops = [copair,copcar,copcar1,copbasic,coppatrol,copsheriff,copswat,copbasic2,copboatshop,tdoc,copk9cars,coptrafficcars,coptraffic,copvicecars,copswatcars];
_donoro = [donor];
_milps = [milp,milpb];
_pmcs = [pmc];
...
if(_shop in _donoro and !((getPlayerUID player) in donatorlevel4)) exitwith {player groupchat "Get out of here, you're no donator!"; closedialog 0};
if(_shop in _pmcs and !((getPlayerUID player) in pmcarray)) exitwith {player groupchat "Get out of here, you're no PMC!"; closedialog 0};
if(_shop in _milps and !((getPlayerUID player) in milarray)) exitwith {player groupchat "This is military police HQ!"; closedialog 0};
...
buttonSetAction [3,  "[""itemkauf"",    lbCurSel 1,   ctrlText 2,   lbData [101, (lbCurSel 1)  ], INV_ActiveBuyShopArray select (lbCurSel 1)]    call shop_buy;"];

buttonSetAction [103, "[""itemverkauf"", lbCurSel 101, ctrlText 102, lbData [101, (lbCurSel 101)], INV_ActiveSellShopArray select (lbCurSel 101)] call shop_sell;"];
