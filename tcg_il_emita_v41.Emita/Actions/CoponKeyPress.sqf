...

{
	...
	if(!isnull _vcl)then{call compile format['[0,0,0,["%3", "public", ["vcl", "%2", %1]]] spawn storage_unit;', _vcl, (typeOf _vcl), format["%1_storage", _vcl]];}; 
	...
	if(iscop) then 
		...
		if(vehicle player == player)then{[0,0,0,["copmenulite"]] spawn cop_menul;};
		if(vehicle player != player)then{[0,0,0,["copmenu"]]  spawn cop_menu;};
		_handled=true;
	...
	[0,0,0,["spielerliste"]] spawn FFG_Dialogs;
